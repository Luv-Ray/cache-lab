#ifndef __DIRECT_CACHE_HH__
#define __DIRECT_CACHE_HH__
...
private:
  ...
  private:
    ...
    /// The object that owns this object (DirectCache)
    DirectCache *owner;
  ...
  public:
     ...
     */
    CPUSidePort(const std::string &name, int id, DirectCache *owner)
     ...
     * This is a convenience function for the DirectCache to send pkts.
     *
     ...
     * from the DirectCache whenever it is unblocked.
     */
    void trySendRetry();
  ...
  public:
     ...
     * Constructor. Just calls the superclass constructor.
     */
    MemSidePort(const std::string &name, DirectCache *owner)
  ...
  std::vector<std::pair<Addr, uint8_t *>> cacheStore;

  /// Cache statistics
protected:
  struct DirectCacheStats : public statistics::Group
  {
    DirectCacheStats(statistics::Group *parent);
...
public:
  /** constructor
   */
  DirectCache(const DirectCacheParams &params);
